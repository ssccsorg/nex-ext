// nex-db-duckdb — DuckDB-backed cold storage for analytical queries.
    ...
    fn exec_intent_query(&self, sql: &str) -> Vec<Intent> {
        ...
        match stmt.query_map([], |row| {
            ...
            Ok(Intent {
                ...
                from_facts: from_facts.iter().map(|s| CoordId::resolve(s)).collect(),
                description,
                creator,
                worker,
                to_fact_id: to_fact_id.map(|s| CoordId::resolve(&s)),
...
impl DuckDbStorage {
    /// Incrementally export hot data newer than the cursor to Parquet
    /// partitions. This is the DuckDB cold-storage flush: the engine's
    /// own paradigm, decoupled from the nexus `flush_pending` surface.
    pub fn flush_since(&self, cursor: &FlushCursor) -> Result<FlushResult, String> {
