use interface_query::QueryCapable;
...
fn test_read_facts_from_parquet() {
    ...
    let fact_1 = state
        .facts
        .iter()
        .find(|f| f.id == CoordId::resolve("fact_1"))
        .unwrap();
    ...
    let fact_2 = state
        .facts
        .iter()
        .find(|f| f.id == CoordId::resolve("fact_2"))
        .unwrap();
...
fn test_read_intents_from_parquet() {
    ...
    let intent_1 = state
        .intents
        .iter()
        .find(|i| i.id == CoordId::resolve("intent_1"))
        .unwrap();
    assert_eq!(intent_1.from_facts, vec![CoordId::resolve("fact_1")]);
    ...
    let intent_2 = state
        .intents
        .iter()
        .find(|i| i.id == CoordId::resolve("intent_2"))
        .unwrap();
    assert_eq!(
        intent_2.from_facts,
        vec![CoordId::resolve("fact_1"), CoordId::resolve("fact_2")]
    );
    assert_eq!(intent_2.description, "do more");
    assert_eq!(intent_2.creator, "admin");
    assert_eq!(intent_2.worker, None);
    assert_eq!(intent_2.to_fact_id, Some(CoordId::resolve("fact_3")));
...
fn test_read_hints_from_parquet() {
    ...
    let hint_1 = state
        .hints
        .iter()
        .find(|h| h.id == CoordId::resolve("hint_1"))
        .unwrap();
    ...
    let hint_2 = state
        .hints
        .iter()
        .find(|h| h.id == CoordId::resolve("hint_2"))
        .unwrap();
...
fn test_filter_by_since() {
    ...
    assert!(
        state
            .facts
            .iter()
            .any(|f| f.id == CoordId::resolve("fact_2")),
    ...
    assert!(
        state
            .facts
            .iter()
            .any(|f| f.id == CoordId::resolve("fact_3")),
    ...
    assert!(
        !state
            .facts
            .iter()
            .any(|f| f.id == CoordId::resolve("fact_1")),
...
fn test_filter_by_fact_ids() {
    ...
    assert_eq!(state.facts[0].id, CoordId::resolve("fact_1"));
    assert_eq!(
...
fn test_scan_partition() {
    ...
    assert_eq!(data_1.facts[0].id, CoordId::resolve("fact_p1"));
    assert_eq!(
    ...
    assert_eq!(data_2.facts[0].id, CoordId::resolve("fact_p2"));
    assert_eq!(
...
fn test_read_multiple_parquet_files() {
    ...
    assert!(
        state
            .facts
            .iter()
            .any(|f| f.id == CoordId::resolve("fact_a"))
    );
    assert!(
        state
            .facts
            .iter()
            .any(|f| f.id == CoordId::resolve("fact_b"))
    );
}
...
fn test_full_board_state() {
    ...
    write_parquet(
        ...
        facts_dir.join("data.parquet").to_str().unwrap(),
    );
    ...
    write_parquet(
        ...
        intents_dir.join("data.parquet").to_str().unwrap(),
    );
    ...
    write_parquet(
        ...
        hints_dir.join("data.parquet").to_str().unwrap(),
    );
    ...
    assert_eq!(state.facts[0].id, CoordId::resolve("f_1"));
    assert_eq!(state.intents[0].id, CoordId::resolve("i_1"));
    assert_eq!(state.hints[0].id, CoordId::resolve("h_1"));
}
...
fn test_filter_combined() {
    ...
    assert_eq!(state.facts.len(), 2, "expected 2 facts");
    assert!(state.facts.iter().any(|f| f.id == CoordId::resolve("f_2")));
    assert!(state.facts.iter().any(|f| f.id == CoordId::resolve("f_3")));
}
...
fn test_filter_intent_and_hint_ids() {
    ...
    assert_eq!(state.intents.len(), 1, "expected 1 intent");
    assert_eq!(state.intents[0].id, CoordId::resolve("i_a"));
...
fn test_complex_json_content() {
    ...
    let nested = state
        .facts
        .iter()
        .find(|f| f.id == CoordId::resolve("f_nested"))
        .unwrap();
    ...
    let null_fact = state
        .facts
        .iter()
        .find(|f| f.id == CoordId::resolve("f_null_content"))
        .unwrap();
    assert_eq!(
        null_fact.content,
        Content {
...
fn test_stress_1000_facts() {
    ...
    assert!(
        state
            .facts
            .iter()
            .any(|f| f.id == CoordId::resolve("fact_0"))
    );
    assert!(
        state
            .facts
            .iter()
            .any(|f| f.id == CoordId::resolve("fact_999"))
    );
}
