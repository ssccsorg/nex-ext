// nex-db-duckdb — Incremental flush tests.
//
...
fn test_flush_partition_isolation() {
    ...
    assert!(!sa2.read_state().facts.is_empty(), "project_a has facts");
    assert!(!sb2.read_state().facts.is_empty(), "project_b has facts");
}
...
fn test_flush_preserves_original_data() {
    ...
    assert_eq!(before.facts[0].id, CoordId::resolve("f_orig"));
    ...
    assert!(
        after
            .facts
            .iter()
            .any(|f| f.id == CoordId::resolve("f_orig")),
