// nex-ext-core: common query contract for the nex-ext storage engines.
//
// This is the QueryCapable spec, moved from the nexus workspace
// (interface/query). DB and graph query specs live with the engines that
// implement them, not in the nexus main repository: the nexus-chton axis
// owns coordinate semantics, while nex-ext owns queryable storage.
//
// The types describe a tabular query against cold storage (Parquet-backed
...
/// (DuckDB dialect over parquet-backed FIH views) lives in nex-duckdb.
/// ColdQuery is the intended endpoint of the cold-routing pipeline.
